...
contract('LeveragedAMM', (accounts) => {
    ...
    it('Estimate eth position should calculate position by AMM with leverage.', async () => {
        let ethPosition = await lAmm.calculateEthPosition.call(200, 5)
    ...
    it('Estimate eth position should revert while leverage exceed max value.', async () => {
        await catchRevert(lAmm.calculateEthPosition.call(50, 11))
    })

    it('Estimate eth position should revert while leveraged usdc exceed reserve usdc.', async () => {
        await catchRevert(lAmm.calculateEthPosition.call(4000, 2))
    })

    it('Estimate usdc position should calculate position by AMM with leverage.', async () => {
        let usdcPosition = await lAmm.calculateUsdcPosition.call(web3.utils.toWei('0.01', 'ether'), 8)
        assert.equal(usdcPosition.valueOf(), 193)
    })

    it('Estimate usdc position should revert while leverage exceed max value.', async () => {
        await catchRevert(lAmm.calculateUsdcPosition.call(web3.utils.toWei('0.01', 'ether'), 12))
    })

    it('Estimate usdc position should revert while leveraged eth exceed reserve eth.', async () => {
        await catchRevert(lAmm.calculateUsdcPosition.call(web3.utils.toWei('1.0', 'ether'), 3))
    })

   
    it('Calculate the amount of ETH to pay for leveraging exact amount of USDC.', async () => {
        let ethPosition = await lAmm.calculateEthForExactUsdcPosition.call(5000, 5)
        assert.equal(web3.utils.fromWei(ethPosition.valueOf(), 'ether'), 0.5)
    })

    it('Calculate the amount of ETH to pay for leveraging exact amount of USDC. Should revert while leverage exceed max value.', async () => {
        await catchRevert(lAmm.calculateEthForExactUsdcPosition.call(50, 11))
    })

    it('Calculate the amount of ETH to pay for leveraging exact amount of USDC. Should revert while leveraged usdc exceed reserve usdc.', async () => {
        await catchRevert(lAmm.calculateEthForExactUsdcPosition.call(20000, 2))
    })

    it('Calculate the amount of USDC to pay for leveraging exact amount of ETH.', async () => {
        let usdcPosition = await lAmm.calculateUsdcForExactEthPosition.call(web3.utils.toWei('0.08', 'ether'), 8)
        assert.equal(usdcPosition.valueOf(), 25)
    })

    it('Calculate the amount of USDC to pay for leveraging exact amount of ETH. Should revert while leverage exceed max value.', async () => {
        await catchRevert(lAmm.calculateUsdcForExactEthPosition.call(web3.utils.toWei('0.012', 'ether'), 12))
    })

    it('Calculate the amount of USDC to pay for leveraging exact amount of ETH. Should revert while leveraged eth exceed reserve eth.', async () => {
        await catchRevert(lAmm.calculateUsdcForExactEthPosition.call(web3.utils.toWei('10', 'ether'), 3))
