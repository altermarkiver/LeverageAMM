...
contract LeveragedAMM is Ownable {
    ...
    function _calculateLeverageUsdc(uint256 ethIn) internal view returns(uint256) {
        require(ethIn < reserveEth, "LeveragedAMM: out of reserve eth.");
        return (reserveUsdc * reserveEth) / (reserveEth - ethIn) - reserveUsdc;
    }

    function _calculateLeverageEth(uint256 usdcIn) internal view returns(uint256) {
        require(usdcIn < reserveUsdc, "LeveragedAMM: out of reserve eth.");
        return (reserveUsdc * reserveEth) / (reserveUsdc - usdcIn) - reserveEth;
    }
    ...
    function calculateEthPosition(uint256 usdc, uint256 leverage) public view returns(uint256) {
        require(leverage <= maxLeverage, "LeveragedAMM: max leverage is 10.");
        return _calculateSwappedEth(usdc * leverage);
    }

    function calculateUsdcPosition(uint256 eth, uint256 leverage) public view returns(uint256) {
        require(leverage <= maxLeverage, "LeveragedAMM: max leverage is 10.");
        return _calculateSwappedUsdc(eth * leverage);
    }

    function calculateEthForExactUsdcPosition(uint256 usdc, uint256 leverage) public view returns(uint256) {
        require(leverage <= maxLeverage, "LeveragedAMM: max leverage is 10.");
        return _calculateLeverageEth(usdc / leverage);
    }

    function calculateUsdcForExactEthPosition(uint256 eth, uint256 leverage) public view returns(uint256) {
        require(leverage <= maxLeverage, "LeveragedAMM: max leverage is 10.");
        return _calculateLeverageUsdc(eth / leverage);
    }
